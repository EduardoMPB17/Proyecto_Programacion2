...
def botonInfoB():
    texto = Label(root, text=infoBarco,width=40,height=16,bg="pale turquoise").grid(row= 4,column=1)
def botonInfoT():
    texto = Label(root, text=infoTren,width=40,height=16,bg="pale turquoise").grid(row= 4,column=2)
def botonInfoA():
    texto = Label(root, text=infoAvion,width=40,height=16,bg="pale turquoise").grid(row= 4,column=3)
def botonInfoC():
    texto = Label(root, text=infoCamion,width=40,height=16,bg="pale turquoise").grid(row= 4,column=4)


infB = ["holamcckdkdkdkd","ddd","ddd","ddd","ddd","ddd","dd2"]
infT = [22]
infA = ["qq"]
infC = ["qq"]

infPdf_total = []#lista donde se guardara cada informacion
...
pdf = Button(root, text="Exportar a PDF", bg="red", width=15, height=2 , borderwidth=10,command=exportarPDF).grid(row=6,column=3)

# Label de informacion
labelti= Label(root, text="Optimizador de Contenedores\n1):Seleccione un archivo\n2):Luego toque una Imagen para ver mas Información",bg="pale green", width=45, height=3,font=('Helvetica', 14, 'bold')).place(x=280,y=0)
