...
class Transporte:
    ...
    def cContenedores(self, tl):
        op = tl // self.tonelaje
        print(
            f"se requieren {op} contenedores {self.tipo_carga} de tamaño {self.tamanho}")
        return op
...
# los cuales son 3, en este caso usaríamos esta función para: Cont.
# Normales, Cont. Refrigerados y Estanques
...
# Tales como la cantidad de cada vehículo, el total general de vehículos y
# el costo
def totalSUM(a, b, c):
    ...
    monto = (bs * barco.costo) + (ts * tren.costo) + \
        (ar * avion.costo) + (cs * camion.costo)
...
def openfile():
    try:
        ...
        # asegura que no se repita informacion,al ejecutar denuevo el programa
        elimina = cursor.execute("DELETE FROM personas")
        conexion.commit()
        sep = archivo.split("/")
        return str(sep[-1])
...
# Ya que si no lo filtrabamos, filedialog nos devolvería el directorio
# completo.
file = openfile()
...
def main():
    ...
    # ya que por defecto estás imagenes realmente son grandes y la interfaz
    # quedaría desigual.
    ...
    def exportarPDF():
        screenshot = pyautogui.screenshot(
            region=(30, 110, 1450, 910))  # dimensiones de la captura
        screenshot.save("captura.png")  # guarda captura
        screenshot.show
        ...
        # pone captura con dimensiones acorde al pdf
        pdf.image("captura.png", w=200, h=250)
    ...
    def CambiaCostos():
        ...
        def cambia():
            ...
            camion.costo = x4
            messagebox.showinfo(
                message=f"El Costo del Tren ahora es:{tren.costo}\nEl Costo del Barco ahora es:{barco.costo}\nEl Costo del Avion ahora es:{avion.costo}\nEL Costo del Camion ahora es:{camion.costo}",
                title="Costos Actualizados")
            ventana.destroy()  # cierra la ventana
            main()  # vuelve a recalcular los costos

        etiqueta = Label(
            ventana,
            text="Ingresa Los Nuevos Costos $",
            font=("Arial"),
            bg="wheat1")
    ...
    # Botones de Imagenes de los vehiculos
    b = loadIMG("barco.png")
    ...
    op = Button(
        root,
        text="Abrir otro Archivo",
        bg="green",
        width=20,
        height=4,
        borderwidth=5,
        relief="groove",
        command=openfile).grid(
        row=0,
        column=1)
    ...
    cambiaC_T = Button(
        root,
        text="Modificar costos",
        bg="yellow2",
        width=15,
        height=2,
        borderwidth=10,
        command=CambiaCostos).grid(
        row=1,
        column=5)
    ...
    labelti = Label(
        root,
        text="Optimizador de Contenedores\n1):Seleccione un archivo\n2):Toque una Imagen para ver mas Información",
        bg="pale green",
        width=40,
        height=3,
        font=(
    ...
    labelCostos = Label(
        root,
        text="Toque el Boton de abajo\n para modificar los costos",
        bg="aqua",
        width=30,
        height=5).grid(
        row=0,
    ...
    labelCantidatT = Label(
        root,
        text=f"Cantidad total de Vehículos es: {totalVeh}",
        bg="skyblue1",
        width=30,
        height=5,
        relief="flat").grid(
    ...
    labelCBarco = Label(
        root,
        text=f"La cantidad Total de Barcos Es: {bs}",
        bg="aquamarine",
        width=30,
        height=2,
        font=(
    ...
    # Label de costo
    labelCostoTotal = Label(
        root,
        text=f"Costo Total:$ {monto} Pesos",
        bg="gold2",
        width=30,
        height=2,
        font=(
    ...
    labelTotalesP = Label(
        root,
        text=f"Total Costos Parciales: {parciales}",
        bg="gold1",
        width=30,
        height=5,
        font=(
